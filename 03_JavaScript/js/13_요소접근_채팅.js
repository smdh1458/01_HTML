function readValue() {
  ...
  //id를 작성하더라도 css에서 작성하는 아이디 표기방법 사용
  //                                  #아이디속성명 들어감
  ...
  if (inputChat.value.trim().length == 0) {
    ...
    alert("채팅 내용을 입력 해주세요."); // 장난치지 말고 내용적어라
    //채팅 내용을 입력 안했으면 돌려보내기
    return;
  }
...
// addEventListener 이용해서 keyup이라는 이벤트를 확인하고
// 이외 다양한 이벤트를 알아볼 것
function 키누르기(event) {
  if (event.key === "Enter") {
    readValue();
